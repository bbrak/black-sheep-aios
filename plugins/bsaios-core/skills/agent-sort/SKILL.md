...
Judge each borderline component on four dimensions, not gut feel:

- **Actionability**: does it give code/commands/steps usable right away, or just general advice?
- **Scope fit**: is the trigger aligned with what it covers, not too broad or too narrow?
- **Uniqueness**: does it add value not already covered by another DAILY component, CLAUDE.md, or MEMORY.md?
- **Currency**: do its technical references (CLI flags, APIs, package names) still hold in this repo's actual toolchain?

Justifications must be self-contained and decision-enabling, not restatable as "unchanged" or "overlaps":

- Bad: `"Overlaps with another skill"`
- Good: `"52 lines duplicate frontend-patterns' Tailwind section; only the Vitest command block is unique — keep that as a note in frontend-patterns instead of installing both."`
