...
5. **Descoberta dinâmica de teammates** — nunca hardcode nomes. Sempre enumere via `~/.claude/skills/team-os/scripts/list-teammates.sh`, que cobre os 4 escopos: projeto (`.claude/agents/`), usuário (`~/.claude/agents/`), plugins habilitados (`{plugin}:{agente}`) e built-ins do harness (`general-purpose`, `Explore`, `Plan`). Nomes citados nos fluxos abaixo (dev-architect etc.) são **preferências quando existem**, nunca requisitos — na ausência, escolher o equivalente mais próximo do catálogo.
...
Rodar `~/.claude/skills/team-os/scripts/preflight.sh`. Se retornar erro, parar e mostrar a mensagem ao usuário. Nunca pular essa etapa.

### Etapa 1 — Detectar estado

Rodar `~/.claude/skills/team-os/scripts/detect-state.sh`. Retorna um de 4 estados:
...
Rodar `~/.claude/skills/team-os/scripts/audit-teammate-compliance.sh`. O gate cobre apenas agentes de projeto e de usuário (os enroláveis) — agentes de plugin e built-ins não são auditados nem recebem `*enroll` (arquivos de plugin vivem em cache sobrescrito a cada update; built-ins não têm `.md`). Se algum teammate falhar:
```
...
Rodar `~/.claude/skills/team-os/scripts/list-teammates.sh`. Retorna o catálogo completo dos 4 escopos:
```
- dev-analyst (Research, CVE, library comparison) [project]
- dev-architect (Architecture, ADRs, stories) [project]
- architect (System design, scalability) [user]
- typescript-reviewer (Type safety, async correctness) [user]
- vercel:deployment-expert (Deploy, CI/CD, rollbacks) [plugin:vercel]
- Explore (busca read-only em larga escala) [builtin]
- ... (todos os agentes disponíveis, sem filtro de prefixo)
```

Classificar cada candidato em dois tiers:
- **[team-ready]** — project/user com contrato instalado e `SendMessage` no `tools:`. Pode assumir qualquer papel, inclusive escrita coordenada via smart-memory.
- **[consultor]** — plugin (`{plugin}:{nome}`) e built-ins. Não são enroláveis; use para tarefas autocontidas (review, research, audit, plano) onde o resultado volta ao lead como task concluída. Não dar a consultores tarefas de escrita concorrente em arquivos compartilhados.

**Checagem anti-mudo:** antes de compor, confirmar que o candidato tem `SendMessage` disponível (frontmatter `tools:` do `.md`, ou all-tools nos built-ins/plugins sem restrição). Agente sem SendMessage fica mudo pro lead — só aceitável como consultor de task única.
...
**Limite de composição:** máx 5 teammates por time. Além disso, retornos diminuem e o overhead de coordenação (SendMessage, smart-memory) supera o ganho. Se o objetivo exigir mais, quebrar em stories e rodar `*dispatch` em ondas.

**Colisão de write-surface antes de confirmar:** para cada par de teammates propostos, checar se os arquivos/áreas que cada um vai tocar se sobrepõem. Se dois teammates escreveriam no mesmo arquivo, serializar (um por vez, via task com dependência) em vez de spawnar ambos em paralelo na Etapa 6 — Agent Teams não resolve conflito de escrita concorrente por conta própria.

Regra geral de bloqueio de arquivo, válida durante todo o ciclo de vida do time: **um arquivo só pode estar sob escrita de um teammate por vez.** Se um segundo teammate precisar do mesmo arquivo, ele aguarda via task dependency — nunca escreve em paralelo "torcendo para não colidir".

### Padrões de execução destilados (de writing-plans/SDD, obra/superpowers MIT)

- **Formato de task/story para teammates:** escreva como se fosse para um júnior entusiasmado
  sem contexto e sem gosto — passos de 2–5 min, paths de arquivo exatos, código/comando completo
  quando relevante. **Proibido "TBD", "etc." e placeholders** — se você não sabe, a task não
  está pronta pra despachar.
- **Model tiering:** tarefa mecânica/bem especificada → teammate em modelo menor (haiku/sonnet);
  decisão arquitetural, débug aberto ou review de risco → modelo maior. Declare o modelo na
  composição quando divergir do default.
- **Review em 2 estágios, reviewers frescos:** primeiro *spec compliance* (fez o que a story
  pede?), depois *qualidade* (está bem feito?). Usar agente NOVO em cada rodada de fix — reviewer
  que já viu o código ancora e aprova o próprio viés.
- **Ledger durável anti-compaction:** progresso por task vive em `docs/smart-memory/` (stories +
  `shared-context.md`), NUNCA só no chat. Regra prática: qualquer estado que doeria perder num
  `/compact` vai pra arquivo no mesmo turno em que surge.
...
- Registrar transições relevantes (dispatch, início, blocker, retomada, conclusão) na smart-memory (`shared-context.md` / `teams-log`) quando fizer sentido para coordenação
...
Dependendo do objetivo (nomes são preferências — na ausência, usar o equivalente do catálogo conforme Regra 5):
- Se envolve arquitetura/stories novas → `SendMessage(dev-architect, "{objetivo}. Quebre em stories e popule backlog em docs/smart-memory/stories/")` (fallback: `architect` [user])
- Se é bug/hardening → `SendMessage(dev-dev-delta, ...)` (fallback: `silent-failure-hunter` [user])
- Se é research → `SendMessage(dev-analyst, ...)` (fallback: `general-purpose` [builtin])
...
Baseado nos sinais, escolher do catálogo de `list-teammates.sh` (4 escopos) quem cobre cada papel:
- **Papel modules/arquitetura** — preferir `dev-architect`; fallback: `architect` [user], `Plan` [builtin]
- **Papel tech stack/conventions** — preferir `dev-analyst`; fallback: `general-purpose` [builtin], `Explore` [builtin]
- **Se DB detectado** → preferir `dev-data-engineer`; fallback: `database-reviewer` [user]
- **Se frontend detectado** → preferir `dev-ux`; fallback: `react-reviewer` [user]

Se nenhum candidato cobrir um papel, seguir apenas com os disponíveis. Nunca bloqueia.
...
Rodar em paralelo:
```bash
~/.claude/skills/team-os/scripts/audit-smart-memory.sh
~/.claude/skills/team-os/scripts/audit-teammate-compliance.sh
```
...
Adicionar a seção "Contrato com team-os" no topo do corpo do prompt (depois do frontmatter, antes do título H1) de `.claude/agents/{agente}.md` ou `~/.claude/agents/{agente}.md`.

**Só funciona em agentes de projeto ou usuário.** Agente de plugin não pode ser enrolled — o cache do plugin é sobrescrito a cada update. Se um agente de plugin merece virar team-ready, o caminho é **vendorizar**: copiar o `.md` do cache para `~/.claude/agents/`, adicionar `SendMessage` ao `tools:` e `memory: user`, e então rodar `*enroll` na cópia.
