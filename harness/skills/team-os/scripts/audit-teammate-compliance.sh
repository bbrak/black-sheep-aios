...
check_agent() {
  ...
  # Check 5: SendMessage habilitado no campo tools: do frontmatter
  # Mencionar no texto não basta — sem a tool habilitada o agente fica mudo pro lead.
  local frontmatter
  frontmatter=$(awk '/^---$/{c++; if(c==2)exit} c==1' "$file")
  if ! echo "$frontmatter" | grep -qi "SendMessage"; then
    issues+=("SendMessage ausente do campo 'tools:' — agente não consegue notificar o lead")
  fi
