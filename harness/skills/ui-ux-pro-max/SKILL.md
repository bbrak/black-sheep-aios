...
### Local extension — anti-slop (Black Sheep AIOS)

Local addition, not from upstream. Rules against generic "AI slop" output. (Purposeless glassmorphism is already covered upstream by `blur-purpose` in §4 Style Selection; decorative-only animation by `motion-meaning` in §7 Animation — not duplicated here.)

| Rule | Standard | Avoid | Why It Matters |
|------|----------|--------|----------------|
| **Opinionated Gradients Only** | Use a gradient only when it reinforces the brand palette and has a clear purpose (depth, focus). | Gratuitous gradients on every surface; default purple-to-blue (`#667eea`→`#764ba2`) or pink-to-orange presets. | Generic gradient defaults are the fastest visual tell of unreviewed AI output. |
| **Custom Hero Composition** | Build hero layout from the product's actual content and hierarchy. | Generic centered-text-over-gradient hero, or stock "Welcome to [App Name]" copy. | Signals a templated, unreviewed page rather than a considered first impression. |
| **Customized Component Theme** | Adapt the design system's default theme (Shadcn, Material, etc.) to the product's palette, radius, and type scale. | Shipping a component library's default theme untouched. | Untouched defaults are immediately recognizable as unstyled scaffolding. |
| **Real or Purpose-Built Imagery** | Use real product screenshots, illustrations, or intentionally sourced assets. | Placeholder images from Unsplash/Lorem Picsum or generic stock photography. | Placeholder imagery undermines credibility and rarely matches brand tone. |
| **Content-Driven Layout** | Shape grids/cards around the actual data and its hierarchy. | Generic identical-card grids with no variation for content type or importance. | Uniform grids ignore content hierarchy and read as unfinished. |
| **Restrained Decorative SVG** | Use decorative SVG patterns sparingly and tie them to meaning. | Generic "AI-generated" decorative SVG backgrounds (blobs, waves, mesh noise) as filler. | Decoration without purpose adds visual noise instead of polish. |
...
> Vendorizado de nextlevelbuilder/ui-ux-pro-max-skill (MIT, (c) 2024 Next Level Builder) — v2.10.2 (commit 12b486b22e67f5d887962ef8351c1ac863bfaeb9). Extensão local: seção anti-slop demarcada.
