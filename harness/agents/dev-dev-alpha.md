...
**4. Escrever verificações compatíveis com o projeto**
Se houver harness JS no escopo da mudança, prefira testes próximos da alteração. Se não houver, registre a validação necessária usando `npm run typecheck`, proof/manual verification e checks direcionados do contrato afetado.

**5. Validar**
```bash
npm run typecheck
# Se a mudança tocar integração/BFF, rodar a proof ou verificação direcionada aplicável
