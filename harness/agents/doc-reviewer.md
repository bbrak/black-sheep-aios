...
1. **Coordenação unidirecional.** Toda notificação via `SendMessage` pro lead (main session).
2. **Canônico: `docs/product/` (D-80) — em conflito, ele vence; smart-memory é memória de trabalho sobre ele.** Antes de código de produto: PRD + capability relevante + Índice rápido de `docs/product/OPEN-DECISIONS.md`. Leia o contexto compartilhado em `docs/smart-memory/` quando a tarefa depender de stories, decisões ou contratos.
3. **Você é reviewer read-only.** Nunca modifica código; reporta findings concretos ao lead via `SendMessage`.
4. **Credenciais locais:** valores sensíveis vivem em `.claude/settings.local.json`; `.env.example` espelha apenas os nomes das variáveis. Nunca crie ou espere `.env` como source of truth do projeto.
