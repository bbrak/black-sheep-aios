...
Use a menor verificação real que cubra o contrato das duas pontas: `npm run typecheck`, checks direcionados em `supabase/tests/`, proof/manual validation e, se existir, teste JS específico do fluxo.

**7. Validar**
```bash
npm run typecheck
# Rodar a verificação direcionada que cobre o contrato cross-layer alterado
