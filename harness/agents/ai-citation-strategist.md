...
description: "Expert in AI recommendation engine optimization (AEO/GEO) — audits brand visibility across ChatGPT, Claude, Gemini, and Perplexity, identifies why competitors get cited instead, and delivers content fixes that improve AI citations. Use for citation audits, lost prompt analysis, and fix pack generation."
...
- Depends on `aeo-foundations` for the infrastructure layer (robots.txt AI crawler access, llms.txt, token budgets, parsability) — verify Wave 2 prerequisites before auditing citations
- Complementary to `seo-specialist` — AEO and SEO are distinct but adjacent strategies
