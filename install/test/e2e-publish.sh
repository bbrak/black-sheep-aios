...
echo "== 0) monta o remote (snapshot do repo atual) + tags stable/latest =="
mkdir -p "$ORIGIN"
tar -C "$REPO" --exclude='./.git' --exclude='./.claude' -cf - . | tar -C "$ORIGIN" -xf -
G init -q; G add -A; G commit -qm "base"; G tag -f stable >/dev/null; G tag -f latest >/dev/null
BASE_V="$(node -e "console.log(require('$ORIGIN/install/manifest.json').version)")"       # versao atual do repo
NEXT_V="$(node -e "const p='$BASE_V'.split('.').map(Number);p[1]++;p[2]=0;console.log(p.join('.'))")"  # o que o OWNER publica

echo "== 1) colaborador instala v1.0.0 =="
mkdir -p "$TEAM"
bash "$REPO/install/install.sh" --claude-home "$TEAM" --dry-run >/dev/null 2>&1
node -e "const j=require('$TEAM/.bsaios/version.json'); process.exit(j.product_version==='$BASE_V'?0:1)" && assert "instalado v$BASE_V" 1 || assert "instalado v$BASE_V" 0
...
echo "== 2) OWNER publica: skill nova + bump v$NEXT_V + sync + tag stable avanca =="
SKILL="$ORIGIN/plugins/bsaios-core/skills/e2e-demo-skill"
mkdir -p "$SKILL"
printf -- '---\nname: e2e-demo-skill\ndescription: skill de teste e2e\n---\n\nDemo.\n' > "$SKILL/SKILL.md"
node -e "const f='$ORIGIN/install/manifest.json';const j=require(f);j.version='$NEXT_V';require('fs').writeFileSync(f,JSON.stringify(j,null,2)+String.fromCharCode(10))"
node "$ORIGIN/install/lib/sync-manifest.js" --write >/dev/null
# entrada de CHANGELOG para o delta
node -e "const fs=require('fs'),f='$ORIGIN/CHANGELOG.md';const s=fs.readFileSync(f,'utf8').replace('## [Não lançado]','## [Não lançado]\n\n## [$NEXT_V] — e2e\n\n### Adicionado\n- Skill e2e-demo-skill (teste).');fs.writeFileSync(f,s)"
G add -A; G commit -qm "feat: e2e-demo-skill (v$NEXT_V)"; G tag -f stable >/dev/null
echo "  stable agora aponta v$NEXT_V com a skill nova"
...
echo "$BANNER" | grep -q "v$NEXT_V disponivel" && assert "banner anuncia v$NEXT_V" 1 || assert "banner anuncia v$NEXT_V" 0
...
A("versao carimbada", require(T+"/.bsaios/version.json").product_version==="'$NEXT_V'");
